...
impl<'info> SwapTickSequence<'info> {
    ...
    /// - `tick_spacing` - A u8 integer of the tick spacing for this ai_dex
    ///
    /// # Returns
    /// - `&Tick`: A reference to the desired Tick object
    /// - `TickArrayIndexOutOfBounds` - The provided array-index is out of bounds
    /// - `TickNotFoundError`: - The provided tick-index is not an initializable tick index in this AiDex w/ this tick-spacing.
    pub fn get_tick(
        &self,
        array_index: usize,
        tick_index: i32,
        ...
        self.arrays
            .get(array_index)
            .ok_or(ErrorCode::TickArrayIndexOutOfBounds.into())
            .and_then(|array| array.get_tick(tick_index, tick_spacing))
    }
    ...
    pub fn update_tick(
        ...
        if let Some(array) = self.arrays.get_mut(array_index) {
            array.update_tick(tick_index, tick_spacing, update)?;
            Ok(())
        } else {
            Err(ErrorCode::TickArrayIndexOutOfBounds.into())
        }
    }

    /// Get the offset of the Tick object at the given tick-index & tick-spacing
    ///
    /// # Parameters
    /// - `array_index` - the array index that the tick of this given tick-index would be stored in
    /// - `tick_index` - the tick index the desired Tick object is stored in
    ...
    ///
    /// # Returns
    /// - `Result<isize>`: The offset of the desired Tick object
    /// - `TickArrayIndexOutOfBounds` - The provided array-index is out of bounds
    pub fn get_tick_offset(
        ...
        self.arrays
            .get(array_index)
            .ok_or(ErrorCode::TickArrayIndexOutOfBounds.into())
            .and_then(|array| array.tick_offset(tick_index, tick_spacing))
    }
    ...
    /// - `InvalidTickArraySequenceErrorIndexError` - The swap loop provided an invalid array index to query the next tick in.
    /// - `InvalidTickArraySequenceError`: - User provided tick-arrays are not in sequential order required to proceed in this trade direction.
    ...
    pub fn get_next_initialized_tick_index(
        ...
        // Iterate over the tick arrays starting from the provided array index
        while let Some(next_array) = self.arrays.get(array_index) {
            // Check if the next initialized tick index exists in the current array
            if let Some(next_index) = next_array.get_next_init_tick_index(search_index, tick_spacing, a_to_b)? {
                return Ok((array_index, next_index));
            }

            // Check if the trade direction is from A to B and the current array is the minimum tick array
            if a_to_b && next_array.is_min_tick_array() {
                return Ok((array_index, MIN_TICK_INDEX));
            }
            // Check if the trade direction is from B to A and the current array is the maximum tick array
            else if !a_to_b && next_array.is_max_tick_array(tick_spacing) {
                return Ok((array_index, MAX_TICK_INDEX));
            }

            // Check if we have reached the last array
            if array_index + 1 == self.arrays.len() {
                // If the trade direction is from A to B, return the start tick index of the last array
                if a_to_b {
                    return Ok((array_index, next_array.start_tick_index));
                }
                // If the trade direction is from B to A, return the last tick index of the last array
                else {
            ...
            // Update the search index and array index for the next iteration
        ...
        Err(ErrorCode::InvalidTickArraySequenceErrorIndexError.into())
    }

    pub fn get_start_tick_index(&self, array_index: usize) -> i32 {
        return self.arrays[array_index].start_tick_index;
    }
}
...
mod swap_tick_sequence_tests {
    ...
    mod modify_ticks {
        ...
        fn modify_tick_uninitializable_tick() {
            ...
            for uninitializable_search_tick in uninitializable_tick_indices {
                ...
                assert_eq!(result.unwrap_err(), ErrorCode::TickNotFoundError.into());
                ...
                assert_eq!(update_result.unwrap_err(), ErrorCode::TickNotFoundError.into());
            }
        }
    ...
    mod a_to_b {
        ...
        #[test]
        #[should_panic(expected = "InvalidTickArraySequenceError")]
    ...
    /// Expected:
    ///     - Panic on InvalidTickArraySequenceError on 1st array
    ...
    ///     - Panic on InvalidTickArraySequenceError when search-sequence is not in array 1's range
