...
use crate::state::{
    tick::*, tick_builder::TickBuilder, ai_dex_builder::AiDexBuilder, TickArray, AiDexPool,
};
use crate::state::{AiDexRewardInfo, NUM_REWARDS};
...
pub fn assert_swap(swap_update: &PostSwapUpdate, expect: &SwapTestExpectation) {
    ...
    assert_eq!(
        AiDexRewardInfo::to_reward_growths(&swap_update.next_reward_infos),
...
pub fn build_filled_tick_array(start_index: i32, tick_spacing: u16) -> Vec<TestTickInfo> {
    (0..TICK_ARRAY_SIZE)
        .map(|n| start_index + n * tick_spacing as i32)
        .filter(|&index| index >= MIN_TICK_INDEX && index < MAX_TICK_INDEX)
        .map(|index| TestTickInfo {
            index,
            liquidity_net: -5,
            ..Default::default()
        })
...
impl SwapTestFixture {
    pub fn new<'info>(info: SwapTestFixtureInfo) -> SwapTestFixture {
        ...
        let direction = if info.a_to_b { -1 } else { 1 };
    
        for (array_index, array) in array_ticks.iter().enumerate() {
            let array_start_tick_index = info.start_tick_index
                + info.tick_spacing as i32 * TICK_ARRAY_SIZE * array_index as i32 * direction;
            ...
            if let Some(tick_array) = array {
                for tick in *tick_array {
                    ...
                    if new_ta.update_tick(tick.index, info.tick_spacing, &update).is_err() {
