...
use crate::{
    errors::ErrorCode,
    orchestrator::{
        tick_orchestrator::next_tick_cross_update, ai_dex_orchestrator::next_ai_dex_reward_infos,
    },
    math::*,
    state::*,
...
/// # Arguments
///
/// * `ai_dex` - The AiDex instance representing the pool.
/// * `swap_tick_sequence` - The mutable reference to the SwapTickSequence.
/// * `amount` - The amount to swap.
/// * `sqrt_price_limit` - The square root price limit.
...
/// # Returns
///
/// Returns a `Result` containing the `PostSwapUpdate` struct if the swap is successful, or an `ErrorCode` if there is an error.
pub fn swap(
    ai_dex: &AiDexPool,
    ...
    if sqrt_price_limit < MIN_SQRT_PRICE_X64 || sqrt_price_limit > MAX_SQRT_PRICE_X64 {
        msg!("it is here, in the orchestrator");
        msg!("sqrt_price_limit: {}", sqrt_price_limit);
        return Err(ErrorCode::SqrtPriceOutOfBoundsError.into());
    }

    // Check if the square root price limit is valid based on the swap direction
    if (a_to_b && sqrt_price_limit > ai_dex.sqrt_price)
        || (!a_to_b && sqrt_price_limit < ai_dex.sqrt_price)
    {
    ...
    let mut curr_sqrt_price = ai_dex.sqrt_price;
    let mut curr_tick_index = ai_dex.tick_current_index;
    ...
    // Loop until the amount is fully swapped or the square root price limit is reached
    while amount_remaining > 0 && sqrt_price_limit != curr_sqrt_price {
        // Get the next initialized tick index and array index
        ...
        // Update the remaining amount and calculated amount based on the swap computation
        if amount_specified_is_input {
            amount_remaining = amount_remaining
                .checked_sub(swap_computation.amount_in)
                .ok_or(ErrorCode::AmountRemainingOverflowError)?;
            amount_remaining = amount_remaining
        ...
        // Calculate the fees
        let (next_protocol_fee, next_referral_fee, next_fee_growth_global_input) = calculate_fees(
        ...
        if swap_computation.next_price == next_tick_sqrt_price {
            ...
            if next_tick_initialized {
                let (fee_growth_global_a, fee_growth_global_b) = if a_to_b {
                    (curr_fee_growth_global_input, ai_dex.fee_growth_global_b)
                } else {
                    (ai_dex.fee_growth_global_a, curr_fee_growth_global_input)
                };
                ...
                emit!(SwapTickUpdate {
                    tick_array_start_index: swap_tick_sequence.get_start_tick_index(next_array_index),
    ...
    // Calculate the fee growth based on the swap direction
    let fee_growth = if a_to_b {
        curr_fee_growth_global_input - ai_dex.fee_growth_global_a
    } else {
        curr_fee_growth_global_input - ai_dex.fee_growth_global_b
    };
...
/// Calculates the fees for the swap operation.
///
/// # Arguments
///
/// * `fee_amount` - The amount of fees.
...
/// # Returns
///
/// Returns a tuple containing the next protocol fee and the next fee growth global input.
fn calculate_fees(
    fee_amount: u64,
    ...
    let mut next_referral_fee = curr_referral_fee;
    ...
    if protocol_fee_rate > 0 {
        // Calculate the protocol fee delta
        let protocol_fee_delta = ((global_fee as u128) * (protocol_fee_rate as u128)
            / PROTOCOL_FEE_RATE_MUL_VALUE)
            .try_into()
            .unwrap_or(0);

        // Subtract the protocol fee from the global fee (remaining goes to LPs)
        global_fee = global_fee.wrapping_sub(protocol_fee_delta);

        // Initialize protocol fee remaining after referral reward
        let mut protocol_fee_remaining = protocol_fee_delta;

        // Calculate the referral fee if applicable
        if referrer_reward_fee_rate > 0 {
            // Calculate the referral fee delta
            let referral_fee_delta = ((protocol_fee_delta as u128) * (referrer_reward_fee_rate as u128)
                / REFERRAL_REWARD_FEE_RATE_MUL_VALUE)
                .try_into()
                .unwrap_or(0);

            // Subtract the referral fee from the protocol fee remaining
            protocol_fee_remaining = protocol_fee_remaining.wrapping_sub(referral_fee_delta);

            // Add the referral fee to the next referral fee
    ...
    // Calculate the fee growth global input (for LPs)
    if curr_liquidity > 0 {
        next_fee_growth_global_input = next_fee_growth_global_input
            .wrapping_add(((global_fee as u128) << Q64_RESOLUTION) / curr_liquidity);
    }

    (next_protocol_fee, next_referral_fee, next_fee_growth_global_input)
}


/// Returns a `Result` containing the `TickUpdate` struct and the next liquidity if the calculation is successful, or an `ErrorCode` if there is an error.
/// Calculates the tick update and the next liquidity after a swap operation.
///
/// # Arguments
///
/// * `tick` - The tick to calculate the update for.
/// * `a_to_b` - Indicates the direction of the swap.
...
/// * `reward_infos` - The array of AiDexRewardInfo structs.
///
/// # Returns
///
/// Returns a `Result` containing the `TickUpdate` struct and the next liquidity if the calculation is successful, or an `ErrorCode` if there is an error.
fn calculate_update(
    ...
    // Calculate the signed liquidity net based on the swap direction
    ...
    // Calculate the tick update and the next liquidity
    let update = next_tick_cross_update(tick, fee_growth_global_a, fee_growth_global_b, reward_infos)?;
    msg!("liquidity: {}", liquidity);
    msg!("signed_liquidity_net: {}", signed_liquidity_net);
    let next_liquidity = add_liquidity_delta(liquidity, signed_liquidity_net)?;

    Ok((update, next_liquidity))
}

/// Calculates the next square root prices based on the next tick index, square root price limit, and swap direction.
///
/// # Arguments
///
/// * `next_tick_index` - The next tick index.
/// * `sqrt_price_limit` - The square root price limit.
/// * `a_to_b` - Indicates the direction of the swap.
///
/// # Returns
///
/// Returns a tuple containing the next tick price and the next square root price limit.
fn get_next_sqrt_prices(
    next_tick_index: i32,
    sqrt_price_limit: u128,
    a_to_b: bool,
) -> (u128, u128) {
    // Calculate the next tick price based on the next tick index
    let next_tick_price = sqrt_price_from_tick_index(next_tick_index);

    // Calculate the next square root price limit based on the swap direction
...
mod swap_liquidity_tests {
    use super::*;
    use crate::util::{create_ai_dex_reward_infos, test_utils::swap_test_fixture::*};
    ...
    fn zero_l_across_tick_range_b_to_a() {
        let swap_test_info = SwapTestFixture::new(SwapTestFixtureInfo {
            ...
            reward_infos: create_ai_dex_reward_infos(100, 10),
    ...
    #[should_panic(expected = "InvalidTickArraySequenceErrorIndexError")]
    ...
    #[test]
    #[should_panic(expected = "MultiplicationShiftRightOverflowError")]
    /// A swap in a pool with maximum liquidity that reaches the maximum tick
    /// |__p1_____c1______p1_c2|max
    ///
    /// Expectation:
    /// The swap will error on `TokenLimitExceededError` as it is not possible to increment more than the maximum token allowed.
...
mod swap_sqrt_price_tests {
    ...
    #[test]
    #[should_panic(expected = "SqrtPriceOutOfBoundsError")]
    ...
    #[test]
    #[should_panic(expected = "InvalidSqrtPriceLimitDirectionError")]
...
mod swap_error_tests {
    ...
    #[test]
    #[should_panic(expected = "NoTradableAmountError")]
    ...
    #[test]
    #[should_panic(expected = "AmountCalculationOverflowError")]
