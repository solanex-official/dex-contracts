...
pub fn calculate_modify_liquidity<'info>(
    ...
    // Get the tick at the lower tick index
    // The lower tick value obtained from the `tick_array_lower` at the specified index.
    let tick_lower =
        tick_array_lower.get_tick(position.tick_lower_index, ai_dex.tick_spacing)?;

    // Load the tick array for the upper tick index
    let tick_array_upper = tick_array_upper.load()?;
    // Get the tick at the upper tick index
    let tick_upper =
        tick_array_upper.get_tick(position.tick_upper_index, ai_dex.tick_spacing)?;

    // Calculate the modify liquidity update using the helper function
...
/// Calculates fee and reward growths for a given position.
///
/// # Arguments
///
/// * `ai_dex` - The AiDex instance.
/// * `position` - The position for which to calculate fee and reward growths.
...
/// # Returns
///
/// A tuple containing the updated position and an array of reward infos.
///
/// # Errors
///
/// This function will return an error if calculating fees and rewards for positions with zero liquidity.
...
fn _calculate_modify_liquidity(
    ...
    // Calculate the tick lower update
    ...
    // Calculate the tick upper update
    ...
    // Calculate the fee growths inside the ticks
    ...
    // Calculate the reward growths inside the ticks
    ...
    Ok(ModifyLiquidityUpdate {
        ai_dex_liquidity: next_global_liquidity,
...
/// Calculates the liquidity token deltas for a given position and liquidity delta.
///
/// # Arguments
///
...
/// # Errors
///
/// This function will return an error if the liquidity delta is zero.
pub fn calculate_liquidity_token_deltas(
    ...
    let (delta_a, delta_b) = match current_tick_index {
        _ if current_tick_index < position.tick_lower_index => {
            // current tick below position
            (
                get_amount_delta_a(lower_price, upper_price, liquidity, round_up)?,
                0,
            )
        }
        _ if current_tick_index < position.tick_upper_index => {
            // current tick inside position
            (
                get_amount_delta_a(sqrt_price, upper_price, liquidity, round_up)?,
                get_amount_delta_b(lower_price, sqrt_price, liquidity, round_up)?,
            )
        }
        _ => {
            ...
            (
                0,
                get_amount_delta_b(lower_price, upper_price, liquidity, round_up)?,
            )
        }
    };
...
/// # Arguments
///
/// * `ai_dex` - The mutable reference to the AiDex instance.
/// * `position` - The mutable reference to the Position instance.
/// * `tick_array_lower` - The lower tick array.
/// * `tick_array_upper` - The upper tick array.
...
pub fn sync_modify_liquidity_values<'info>(
    ...
    // Update the upper tick in the tick array
    ...
    // Update the rewards and liquidity in the AiDex instance
    ai_dex.update_rewards_and_liquidity(
...
mod calculate_modify_liquidity_unit_tests {
    ...
    mod xx_position {
        use crate::{orchestrator::liquidity_orchestrator::_calculate_modify_liquidity, util::*};

        // Zero liquidity delta on position with zero liquidity is not allowed
        #[test]
        #[should_panic(expected = "ZeroLiquidityError")]
        ...
        // LiquidityUnderflowError from lower tick (xx_oo)
        #[test]
        #[should_panic(expected = "LiquidityUnderflowError")]
        ...
        // Removing liquidity from position with zero liquidity results in error
        // LiquidityUnderflowError from upper tick (oo_xx)
        #[test]
        #[should_panic(expected = "LiquidityUnderflowError")]
        ...
        // Removing liquidity from position with zero liquidity results in error
        // LiquidityUnderflowError from updating position (oo_oo - not ticks)
        #[test]
        #[should_panic(expected = "LiquidityUnderflowError")]
    ...
    mod xo_position {
        ...
        // Current tick below position
        // AiDex virtual liquidity does not change
        mod current_tick_below {
            use crate::{
                orchestrator::liquidity_orchestrator::_calculate_modify_liquidity, state::*, util::*,
            };
            ...
            fn pos_delta_current_tick_below_xo_lower_xo_upper() {
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(to_x64(3)),
            ...
            fn pos_delta_current_tick_below_xo_lower_oo_upper_underflow() {
                ...
                test.increment_ai_dex_reward_growths_by_time(100);
                test.cross_tick(TickLabel::Upper, Direction::Left);
                // Check crossing an upper tick with liquidity added new ai_dex liquidity
                let liquidity = test.ai_dex.liquidity;
                assert_eq!(liquidity, 110);
                // 1 = 0 + (100/100)
                assert_ai_dex_reward_growths(&test.ai_dex.reward_infos, to_x64(1));
                test.increment_ai_dex_fee_growths(to_x64(10), to_x64(10));
                test.increment_ai_dex_reward_growths_by_time(100);
                test.cross_tick(TickLabel::Lower, Direction::Left);
                // Lower tick has 0 net liquidity, so crossing does not affect ai_dex liquidity
                let liquidity = test.ai_dex.liquidity;
                assert_eq!(liquidity, 110);
                // 1.909 = 1 + (100/110)
                assert_ai_dex_reward_growths(&test.ai_dex.reward_infos, 35216511413445507630);
                ...
                assert_modify_liquidity(
                    &update,
                    &ModifyLiquidityExpectation {
                        // Current tick below position, so does not add to ai_dex liquidity
                        ai_dex_liquidity: 110,
                        // 2.8181 = 1.909 + 0.909
                        ai_dex_reward_growths: create_reward_growths(51986278753181463644),
                        position_update: PositionUpdate {
                            liquidity: 10,
        ...
        // Current tick inside position
        // AiDex virtual liquidity increases
        mod current_tick_inside {
            ...
            fn pos_delta_current_tick_inside_xo_lower_xo_upper() {
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ai_dex_liquidity: 110,
                        ai_dex_reward_growths: create_reward_growths(to_x64(1)),
                        position_update: PositionUpdate {
                            liquidity: 10,
                            ..Default::default()
                        },
            ...
            fn pos_delta_current_tick_inside_xo_lower_oo_upper_underflow() {
                ...
                assert_modify_liquidity(
                    &update,
                    &ModifyLiquidityExpectation {
                        // Current tick inside position, so ai_dex liquidity increases
                        ai_dex_liquidity: 120,
                        // 1.909 = 1 + 0.909
                        ai_dex_reward_growths: create_reward_growths(35216511413445507630),
                        position_update: PositionUpdate {
                            liquidity: 10,
                            // Wrapped underflow -10
                            fee_growth_checkpoint_a: 340282366920938463278907166694672695296,
        ...
        mod current_tick_above {
            ...
            // Simulates the ai_dex current tick moving below the upper tick, accruing fees
            ...
            fn pos_delta_current_tick_above_xo_lower_oo_upper_underflow_owed_amounts_ok() {
                ...
                assert_ai_dex_reward_growths(&test.ai_dex.reward_infos, to_x64(2));
                assert_eq!(
                    test.tick_upper,
                    Tick {
                        initialized: true,
                ...
                // upper tick checkpoints are inverted
                test.increment_ai_dex_fee_growths(to_x64(15), to_x64(15));
                test.increment_ai_dex_reward_growths_by_time(100);
                // 2.83 = 2 + 100/120
                assert_ai_dex_reward_growths(&test.ai_dex.reward_infos, 52265774875510396245);
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ai_dex_liquidity: 100,
                        // 3.83 = 2.83 + 100/100
                        ai_dex_reward_growths: create_reward_growths(70712518949219947861),
                        position_update: PositionUpdate {
                            liquidity: 10,
            ...
            fn pos_delta_current_tick_above_oo_lower_oo_upper_non_zero_checkpoints() {
                // Test fixture is set up to simulate ai_dex at state T1.
                ...
                test.ai_dex.reward_last_updated_timestamp = 200;
    ...
    mod ox_position {
        ...
        mod current_tick_below {
            ...
            fn neg_delta_current_tick_below_oo_lower_oo_upper_non_zero_checkpoints() {
                // Test fixture is set up to simulate ai_dex at state T2.
                ...
                let mut test = LiquidityTestFixture::new(LiquidityTestFixtureInfo {
                    ...
                    reward_infos: create_ai_dex_reward_infos(to_x64(1), to_x64(30)),
                });

                // Time starts at 30_000. Increments of 10_000 seconds with 1000 ai_dex liquidity
                // equate to increments of 10 global rewards.
                test.ai_dex.reward_last_updated_timestamp = 30_000;
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ai_dex_liquidity: 1000,
                        // 40 = 30 + (10000 / 1000)
                        ai_dex_reward_growths: create_reward_growths(737869762948382064640),
                        position_update: PositionUpdate {
                            liquidity: 0,
    ...
    mod oo_position {
        use crate::{orchestrator::liquidity_orchestrator::_calculate_modify_liquidity, state::*, util::*};
    ...
    mod fees_and_rewards {
        ...
        fn accrued_tokens_ok_closed_position_ticks_remain_initialized() {
            // AiDex with 1000 liquidity, fees (a: 100, b: 200) and reward (20)
            ...
            let mut test = LiquidityTestFixture::new(LiquidityTestFixtureInfo {
                ...
                fee_growth_global_b: to_x64(200),
                reward_infos: create_ai_dex_reward_infos(to_x64(1), to_x64(20)),
            });
            ...
            // 20.1 = 20 + (100 / 1000)
            assert_ai_dex_reward_growths(&update.reward_infos, 370779555881561987481);
            assert_eq!(
            ...
            test.increment_ai_dex_fee_growths(to_x64(10), to_x64(20));
            let update = _calculate_modify_liquidity(
                &test.ai_dex,
                &test.position,
                &test.tick_lower,
            ...
            assert_ai_dex_reward_growths(&update.reward_infos, 372456532615535583082);
            assert_eq!(
                update.position_update,
                PositionUpdate {
                    liquidity: 150,
            ...
            assert_modify_liquidity(
                ...
                &ModifyLiquidityExpectation {
                    ...
                    ai_dex_reward_growths: create_reward_growths(374060597317597283222),
                    position_update: PositionUpdate {
                        liquidity: 0,
        ...
        mod global_accumulators_overflow {
            ...
            fn overflow_below_checkpoint_below() {
                ...
                let mut test = LiquidityTestFixture::new(LiquidityTestFixtureInfo {
                    ...
                    reward_infos: create_ai_dex_reward_infos(to_x64(100), u128::MAX - to_x64(5)),
                });
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(u128::MAX - to_x64(4)),
                        position_update: PositionUpdate {
                            liquidity: 1000,
                            ..Default::default()
                        },
                ...
                let fee_growth_global_a = test.ai_dex.fee_growth_global_a;
                let fee_growth_global_b = test.ai_dex.fee_growth_global_b;
                assert_eq!(fee_growth_global_a, u128::MAX - to_x64(100));
                assert_eq!(fee_growth_global_b, u128::MAX - to_x64(100));

                test.apply_update(&update, 100);
                test.increment_ai_dex_fee_growths(to_x64(20), to_x64(20)); // fees at -80
                test.increment_ai_dex_reward_growths_by_time(100);
                // time: 200, rewards at MAX - 3
                assert_ai_dex_reward_growths(
                    &test.ai_dex.reward_infos,
                    u128::MAX - to_x64(3),
                );

                // t2 - cross right, accrue tokens in position
                test.cross_tick(TickLabel::Lower, Direction::Right);
                test.increment_ai_dex_fee_growths(to_x64(20), to_x64(20)); // fees at -60
                test.increment_ai_dex_reward_growths_by_time(100); // 300
                ...
                test.cross_tick(TickLabel::Lower, Direction::Left);
                test.increment_ai_dex_fee_growths(to_x64(70), to_x64(70)); // fees overflow to 10
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(16769767339735956013),
                        position_update: PositionUpdate {
                            liquidity: 1000,
                ...
                let fee_growth_global_a = test.ai_dex.fee_growth_global_a;
                let fee_growth_global_b = test.ai_dex.fee_growth_global_b;
                assert_eq!(fee_growth_global_a, 184467440737095516159);
                assert_eq!(fee_growth_global_b, 184467440737095516159);
            }
            ...
            fn overflow_below_checkpoint_inside() {
                ...
                // t3 - overflow
                test.increment_ai_dex_fee_growths(to_x64(90), to_x64(90)); // fees overflow to 10
                test.increment_ai_dex_reward_growths_by_time(400); // 600
            ...
            fn overflow_below_checkpoint_above() {
                ...
                // t3 - overflow
                test.increment_ai_dex_fee_growths(to_x64(70), to_x64(70)); // fees overflow to 10
                test.increment_ai_dex_reward_growths_by_time(300); // 600
            ...
            fn overflow_inside_checkpoint_below() {
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(11738837137815169209),
                        position_update: PositionUpdate {
                            liquidity: 1000,
            ...
            fn overflow_inside_checkpoint_inside() {
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(
                ...
                // t2 - accrue tokens, overflow
                test.increment_ai_dex_fee_growths(to_x64(110), to_x64(110)); // fees overflow to 10
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(20496382304121724016),
                        position_update: PositionUpdate {
                            liquidity: 1000,
            ...
            fn overflow_inside_checkpoint_above() {
                ...
                assert_modify_liquidity(
                    ...
                    &ModifyLiquidityExpectation {
                        ...
                        ai_dex_reward_growths: create_reward_growths(22546020534533896417),
                        position_update: PositionUpdate {
                            liquidity: 1000,
                            fee_growth_checkpoint_a: to_x64(90),
                            fee_owed_a: 90000,
