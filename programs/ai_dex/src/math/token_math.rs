...
// Referral reward fee rate is represented as a basis point.
// Referral reward fee amount = protocol_fee_amount * referral_reward_fee_rate / 10_000.
// Max referral reward fee rate supported is 15% of the protocol fee.
pub const MAX_REFERRAL_REWARD_FEE_RATE: u16 = 1_500;

// Assuming that REFERRAL_REWARD_FEE_RATE is represented as a basis point
// We want REFERRAL_REWARD_FEE_RATE_MUL_VALUE = 1/REFERRAL_REWARD_FEE_UNIT, so 1e4
pub const REFERRAL_REWARD_FEE_RATE_MUL_VALUE: u128 = 10_000;

// Max reinvestment protocol fee rate is 10% (1000 basis points)
pub const MAX_REINVESTMENT_PROTOCOL_FEE_RATE: u16 = 1_500;
...
pub fn get_amount_delta_a(
    ...
    let quotient = numerator.div(denominator, round_up).0;

    let result = quotient.try_into_u128()?
        .checked_add(if round_up && !numerator.div(denominator, round_up).1.is_zero() { 1 } else { 0 })
        .ok_or(ErrorCode::TokenLimitExceededError)?;

    if result > u64::MAX as u128 {
        return Err(ErrorCode::TokenLimitExceededError);
    }
...
pub fn get_next_sqrt_price_from_a_round_up(
    ...
    if !amount_specified_is_input && liquidity_shift_left.lte(product) {
        return Err(ErrorCode::DivisionByZeroError);
    }
    ...
    match price {
        p if p < MIN_SQRT_PRICE_X64 => Err(ErrorCode::TokenAmountBelowMinimumError),
        p if p > MAX_SQRT_PRICE_X64 => Err(ErrorCode::TokenLimitExceededError),
        p => Ok(p),
    }
}
...
pub fn get_next_sqrt_price_from_b_round_down(
    ...
    let new_sqrt_price = if amount_specified_is_input {
    ...
    new_sqrt_price.ok_or(ErrorCode::SqrtPriceOutOfBoundsError)
}
