...
pub fn add_liquidity_delta(liquidity: u128, delta: i128) -> Result<u128, ErrorCode> {
    match delta {
        ...
        d => liquidity
            .checked_sub(d.abs() as u128)
            .ok_or(ErrorCode::LiquidityUnderflowError),
    }
}
...
pub fn convert_to_liquidity_delta(
    ...
    if liquidity_amount > i128::MAX as u128 {
        ...
        return Err(ErrorCode::ExcessiveLiquidityError.into());
    }

    let delta = liquidity_amount as i128;
    Ok(if positive { delta } else { -delta })
}
...
mod liquidity_math_tests {
    ...
    fn test_invalid_add_liquidity_delta_overflow() {
        ...
        assert_eq!(result.unwrap_err(), ErrorCode::LiquidityOverflowError);
    }

    #[test]
    fn test_invalid_add_liquidity_delta_underflow() {
        let result = add_liquidity_delta(u128::MIN, -1);
        assert_eq!(result.unwrap_err(), ErrorCode::LiquidityUnderflowError);
    }
}
