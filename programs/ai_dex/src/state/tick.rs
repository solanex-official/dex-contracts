...
impl Tick {
    ...
    /// Check that the tick index is a valid start tick for a tick array in this ai_dex
    ...
    ///
    /// # Returns
    /// - `true`: The tick index is a valid start-tick-index for this ai_dex
    /// - `false`: The tick index is not a valid start-tick-index for this ai_dex
    ...
    pub fn check_is_valid_start_tick(tick_index: i32, tick_spacing: u16) -> bool {
        ...
        if Tick::check_is_out_of_bounds(tick_index) {
            ...
            return tick_index == MIN_TICK_INDEX - (MIN_TICK_INDEX % ticks_in_array + ticks_in_array);
        }
...
impl TickArray {
    ...
    /// - `InvalidTickArraySequenceError` - error if `tick_index` is not a valid search tick for the array
    /// - `UnsupportedTickSpacing` - error if the provided tick spacing is 0
    ...
    ///
    /// # Parameters
    /// - `ai_dex` - the tick index the desired Tick object is stored in
    /// - `start_tick_index` - A u8 integer of the tick spacing for this ai_dex
    ///
    /// # Errors
    /// - `InvalidStartTickIndex`: - The provided start-tick-index is not an initializable tick index in this AiDex w/ this tick-spacing.
    pub fn initialize(
        &mut self,
        ai_dex: &AccountLoader<AiDexPool>,
        start_tick_index: i32,
    ) -> Result<()> {
        let ai_dex_data = ai_dex.load()?; // Safely load the zero-copy data

        if !Tick::check_is_valid_start_tick(start_tick_index, ai_dex_data.tick_spacing) {
            msg!("Tick index: {}", start_tick_index);
            return Err(ErrorCode::InvalidStartTickIndex.into());
        }
    ...
    pub fn get_tick(&self, tick_index: i32, tick_spacing: u16) -> Result<&Tick> {
        ...
        {
            return Err(ErrorCode::TickNotFoundError.into());
        }
